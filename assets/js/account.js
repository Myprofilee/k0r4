firebase.auth().onAuthStateChanged((user) => {
  if (user) {
    db.doc(`users/${user.email}`).onSnapshot((doc) => {
      ...
      document.querySelector(".account-username").innerHTML = username;
...
const orderDetails = (orderId) => {
  ...
  const displayOrderDetail = (data) => {
    ...
    <label><a href="/m/account/" class="black-text"><i class="material-icons">arrow_back</i></a></label> 
      <div class="card darken-1">
        <div class="card-content">
          ...
          <div class="card-content">
          <p>Total Pembayaran: ${data.price}</p>
